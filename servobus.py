...
class ServoSerial:
    ...
    def receive_data(self, echo_len, cmd):
        rx_buff = []
        while self.Serial.in_waiting:
            rx = self.Serial.read(self.Serial.in_waiting)
        sleep(0.05)
        # rx = self.Serial.read(start_len + stop_len)
        # print("Received data(RAW): ",rx)
        # print("Received data(DEC): ",list(rx))
        for i in range(echo_len, len(rx)):
            rx_buff.append(rx[i])
        print("Received Frame: ",list(rx_buff))
        if self.rx_check(rx_buff, len(rx_buff), cmd) == SERVO_PACKET_OK:
            return rx_buff
        else:
            return []
    ...
    def Read_MoveTime(self):
        ...
        num = self.transmit_data(packet)
        if num == SERVO_PACKET_OK:
            rx = self.receive_data(SERVO_PACKET_LEN_3+3, SERVO_MOVE_TIME_READ)
    ...
    def Read_MoveTimeWait(self):
        ...
        num = self.transmit_data(packet)
        if num == SERVO_PACKET_OK:
            rx = self.receive_data(SERVO_PACKET_LEN_3+3, SERVO_MOVE_TIME_WAIT_READ)
    ...
    def Read_ServoID(self):
        ...
        num = self.transmit_data(packet)
        if num == SERVO_PACKET_OK:
            rx = self.receive_data(SERVO_PACKET_LEN_3 + 3, SERVO_ID_READ)
    ...
    def Read_AngleOffset(self):
        ...
        num = self.transmit_data(packet)
        if num == SERVO_PACKET_OK:
            rx = self.receive_data(SERVO_PACKET_LEN_7 + 3, SERVO_ANGLE_OFFSET_READ)
    ...
    def Read_AngleLimit(self):
        ...
        num = self.transmit_data(packet)
        if num == SERVO_PACKET_OK:
            rx = self.receive_data(SERVO_PACKET_LEN_3 + 3, SERVO_ANGLE_LIMIT_READ)
        if len(rx) == SERVO_PACKET_LEN_7 + 3:
    ...
    def Read_VinLimit(self):
        ...
        num = self.transmit_data(packet)
        if num == SERVO_PACKET_OK:
            rx = self.receive_data(SERVO_PACKET_LEN_3 + 3, SERVO_VIN_LIMIT_READ)
    ...
    def Read_TempMaxLimit(self):
        ...
        num = self.transmit_data(packet)
        if num == SERVO_PACKET_OK:
            rx = self.receive_data(SERVO_PACKET_LEN_3 + 3, SERVO_TEMP_MAX_LIMIT_READ)
    ...
    def Read_Temp(self):
        ...
        num = self.transmit_data(packet)
        if num == SERVO_PACKET_OK:
            rx = self.receive_data(SERVO_PACKET_LEN_3 + 3, SERVO_TEMP_READ)
    ...
    def Read_Vin(self):
        ...
        num = self.transmit_data(packet)
        if num == SERVO_PACKET_OK:
            rx = self.receive_data(SERVO_PACKET_LEN_3 + 3, SERVO_VIN_READ)
    ...
    def Read_Pos(self):
        ...
        num = self.transmit_data(packet)
        if num == SERVO_PACKET_OK:
            rx = self.receive_data(SERVO_PACKET_LEN_3 + 3, SERVO_POS_READ)
        if len(rx) == SERVO_PACKET_LEN_5 + 3:
    ...
    def Read_ServoOrMotorMode(self):
        ...
        num = self.transmit_data(packet)
        if num == SERVO_PACKET_OK:
            rx = self.receive_data(SERVO_PACKET_LEN_3 + 3, SERVO_OR_MOTOR_MODE_READ)
    ...
    def Read_LedCtrl(self):
        ...
        num = self.transmit_data(packet)
        if num == SERVO_PACKET_OK:
            rx = self.receive_data(SERVO_PACKET_LEN_3 + 3, SERVO_LED_CTRL_READ)
    ...
    def Read_LedError(self):
        ...
        num = self.transmit_data(packet)
        if num == SERVO_PACKET_OK:
            rx = self.receive_data(SERVO_PACKET_LEN_3 + 3, SERVO_LED_ERROR_READ)
        if len(rx) == SERVO_PACKET_LEN_4 + 3:
            return rx[5]
        else:
            print("An error was occured.\n")
